...
(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    define(["../../jslib/matrix_simple"], factory);
    ...
    root.Crystal = factory(root.Matrix);
  }
}(this, function (_Matrix) {
  let Matrix;

  if (typeof require === "undefined" && typeof _Matrix === "function") {
    Matrix = _Matrix;
  } else {
    Matrix = require('../../jslib/matrix_simple.js');
  ...
  var Crystal = function (_melt0, _solidParam, _datom = 0.005) {
  ...
  Crystal.prototype.differentiate = function (finMgN, targetPhase, record) {
    if (finMgN > 100 || finMgN < 0) throw new RangeError();
    let Matrix = Crystal.Matrix;
    ...
    vecB[2][0] = 0;
    vecC[0][0] = 0;
    vecC[1][0] = datom * 0.001//*counter*counter*counter;
    vecC[2][0] = 0;
    ...
    if (_finMgN < 100 / (1 + this.getFeMgRatio("melt") * this.D[targetPhase].Fe_Mg)) {
      var targetMgN = 100;
      var sig = -1;
      var pathName = "descend";
    } else {
      targetMgN = 0;
      sig = 1;
      pathName = "ascend";
    ...
    /* MgO, FeO, SiO2の計算 */
    while (targetMgN * sig < _finMgN * sig && targetMgN >= 0) {	//diff
      ...
      // 液相
      atom.melt.MgO = atom.melt.MgO + sig * atom.solid.MgO;	//diff
      atom.melt.FeO = atom.melt.FeO + sig * atom.solid.FeO;	//diff
      atom.melt.SiO2 = atom.melt.SiO2 + sig * atom.solid.SiO2;	//diff
      ...
      for (let key in trace) {
        elem = trace[key];
        compo.melt[elem] = compo0.melt0[elem] * (Math.pow((1 + sig * F), (D.solid[elem]) - 1));	//diff
        compo.solid[elem] = compo.melt[elem] * D.solid[elem];
        compo.olivine[elem] = compo.melt[elem] * D.olivine[elem];
        compo.orthopyroxene[elem] = compo.melt[elem] * D.orthopyroxene[elem];
      };
      ...
      if (recordFlag) {
        ...
        this.profile[pathName].push({ fraction: F, compo: compoObj }); //diff
    ...
    this.water.before = parseFloat(this.water.after);
    this.water.after = parseFloat(this.water.after / (1 + sig * F)); //diff
